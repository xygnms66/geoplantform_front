...
export type SimpleModelTableData = {
  title?: string;
  subtitle?: string;
  metricLabels: string[];
  rows: SimpleModelRow[];
...
<template>
  ...
  <div v-if="categories" class="ew-wrap">
    ...
    <div class="ew-table-wrap">
      <table class="ew-table">
        <thead>
          <tr>
            <th rowspan="2" class="ew-model-col">模型名称</th>
            <th v-for="cat in categories" :key="cat" colspan="2">{{ cat }}</th>
          </tr>
          <tr>
  ...
  <div v-else-if="metricLabels" class="ew-wrap">
    ...
    <div class="ew-table-wrap">
      <table class="ew-table">
        <thead>
          <tr>
            <th class="ew-model-col">模型名称</th>
            <th v-for="label in metricLabels" :key="label">{{ label }}</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="row in rows" :key="row.model" :class="{ 'ew-highlight-row': row.isHighlight }">
            <td class="ew-model-cell">{{ row.model }}</td>
            <td v-for="label in metricLabels" :key="label" :class="{ 'ew-highlight-cell': row.isHighlight }">
              {{ row.metrics[label] != null ? (row.metrics[label] as number).toFixed(4) : "—" }}
            </td>
          </tr>
        </tbody>
      </table>
  ...
  <div v-else-if="projects" class="ew-wrap">
    ...
    <div
      ...
      :style="{
        display: 'grid',
        gridTemplateColumns: `repeat(${Math.min(projects.length * rows.filter((r) => r.isOverall).length, 4)}, minmax(0, 1fr))`,
        gap: '12px',
        marginBottom: '16px',
      }"
    >
      <template v-for="proj in projects" :key="proj.label">
        <div
          v-for="row in rows.filter((r) => r.isOverall)"
          :key="`${proj.label}-${row.category}`"
        ...
        >
          ...
          <div class="ew-card-bottom">
            <span>AEF {{ fmt(row.scores[proj.aefKey]) }}</span
            ><span v-if="delta(row.scores[proj.aefKey], row.scores[proj.geoKey]) != null" class="ew-improve"
              >+{{ (delta(row.scores[proj.aefKey], row.scores[proj.geoKey]) as number).toFixed(4) }} / +{{
                (
                  ((delta(row.scores[proj.aefKey], row.scores[proj.geoKey]) as number) /
                    (row.scores[proj.aefKey] as number)) *
                  100
                ).toFixed(1)
              }}%</span
            >
    ...
    <div class="ew-table-wrap">
      <table class="ew-table">
        <thead>
          <tr>
            <th rowspan="2" class="ew-cat-col">类别</th>
            <th v-for="proj in projects" :key="proj.label" colspan="3">{{ proj.label }}</th>
          </tr>
          <tr>
            <template v-for="proj in projects" :key="proj.label"
              ><th>AEF</th>
              <th class="ew-geo-head">地溯 GeoOrigin</th>
              <th>Δ 提升</th></template
            >
          </tr>
        </thead>
        <tbody>
          <tr v-for="row in rows" :key="row.category" :class="{ 'ew-overall-row': row.isOverall }">
            ...
            <template v-for="proj in projects" :key="proj.label">
              ...
              <td :class="deltaClass(row.scores[proj.aefKey], row.scores[proj.geoKey])">
                {{ fmtDelta(row.scores[proj.aefKey], row.scores[proj.geoKey]) }}
              </td>
            </template>
          </tr>
        </tbody>
...
.ew-table thead tr:nth-child(2) th {
...
.ew-table tbody tr:nth-child(even) td {
...
.ew-geo-head {
  background: rgba(22, 79, 120, 0.8) !important;
  color: #bfdbfe !important;
}
.ew-geo-cell {
  background: rgba(22, 79, 120, 0.22) !important;
  font-weight: 700;
  color: #93c5fd;
}
.ew-model-col {
...
.ew-highlight-row td {
  background: rgba(22, 57, 100, 0.55) !important;
}
.ew-highlight-row .ew-model-cell {
...
.ew-overall-row td {
  background: rgba(22, 79, 120, 0.28) !important;
  font-weight: 900;
  color: #bfdbfe;
}
.ew-overall-row .ew-geo-cell {
  color: #60a5fa;
}
.ed-positive {
  color: #4ade80;

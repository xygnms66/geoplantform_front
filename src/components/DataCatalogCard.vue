...
const statusText: Record<DataCatalogCardType["status"], string> = {
  candidate: "候选数据", planned: "计划接入", available: "已入库", processing: "处理中", imported: "已接入",
};
    ...
    <div class="dc-card-footer">
      ...
      <div class="dc-status-group">
        ...
        <template v-if="item.storageStatus">
          <button v-if="item.storageStatus === 'local' && item.storagePath" type="button" class="dc-action-button" @click="handleCopyPath" title="复制存储路径">{{ copySuccess ? '✓ 已复制' : '📋 复制路径' }}</button>
          <button v-else-if="item.storageStatus === 'drive' && item.storagePath" type="button" class="dc-action-button" @click="handleDriveClick" title="打开网页">🌐 打开网页</button>
        </template>
...
.dc-status-group a { color: var(--brand); text-decoration: none; }
.dc-status { display: inline-flex; align-items: center; padding: 2px 9px; border-radius: 999px; font-size: 11px; font-weight: 700; }
.dc-status-available, .dc-status-imported { color: #bbf7d0; background: rgba(52, 211, 153, 0.14); border: 1px solid rgba(52, 211, 153, 0.24); }
.dc-status-processing, .dc-status-planned { color: #fde68a; background: rgba(251, 191, 36, 0.12); border: 1px solid rgba(251, 191, 36, 0.22); }
.dc-status-candidate { color: var(--muted); background: rgba(148, 163, 184, 0.1); border: 1px solid var(--line); }
.dc-action-button { padding: 4px 10px; border-radius: 999px; border: 1px solid rgba(96, 165, 250, 0.3); background: rgba(96, 165, 250, 0.1); color: #93c5fd; font: inherit; font-size: 11px; font-weight: 600; cursor: pointer; transition: all 0.18s; }
.dc-action-button:hover { background: rgba(96, 165, 250, 0.18); border-color: rgba(96, 165, 250, 0.5); }
</style>

...
<template>
  ...
  <div v-else style="padding: 2rem; max-width: 1400px; margin: 0 auto">
    ...
    <div v-if="error" style="padding: 1rem; margin-bottom: 2rem; background: #fee; color: #c00; border-radius: 0.5rem">
      {{ error }}
    </div>
    <div
      v-if="statistics"
    ...
    >
      <div class="panel" style="padding: 1.5rem">
        <div style="font-size: 2rem; font-weight: bold; color: #2563eb">{{ statistics.total }}</div>
        <div style="color: #666; margin-top: 0.25rem">总数据集</div>
      </div>
      <div class="panel" style="padding: 1.5rem">
        <div style="font-size: 2rem; font-weight: bold; color: #f59e0b">{{ statistics.by_status.candidate }}</div>
        <div style="color: #666; margin-top: 0.25rem">候选数据集</div>
      </div>
      <div class="panel" style="padding: 1.5rem">
        <div style="font-size: 2rem; font-weight: bold; color: #10b981">{{ statistics.by_status.in_use }}</div>
        <div style="color: #666; margin-top: 0.25rem">使用中</div>
      </div>
      <div class="panel" style="padding: 1.5rem">
        <div style="font-size: 2rem; font-weight: bold; color: #6b7280">{{ statistics.by_status.archived }}</div>
        <div style="color: #666; margin-top: 0.25rem">已归档</div>
      </div>
    </div>
    <div class="panel" style="padding: 1.5rem">
          ...
          <tbody>
            <tr v-for="dataset in datasets" :key="dataset.id" style="border-bottom: 1px solid #e5e7eb">
              ...
              <td style="padding: 0.75rem">
                <div style="font-weight: 500">{{ dataset.title }}</div>
              ...
              <td style="padding: 0.75rem">
                <span
                  style="padding: 0.25rem 0.5rem; background: #e0e7ff; border-radius: 0.25rem; font-size: 0.75rem"
              ...
              <td style="padding: 0.75rem">
                <span
                  v-for="(m, i) in dataset.modality.slice(0, 2)"
                  :key="i"
                  style="
                  ...
                  >{{ m }}</span
                >
                <span v-if="dataset.modality.length > 2" style="font-size: 0.75rem; color: #666"
                  >+{{ dataset.modality.length - 2 }}</span
                >
              </td>
              <td style="padding: 0.75rem">
                <span
                  v-if="dataset.storageStatus"
                  :style="{
                    padding: '0.25rem 0.5rem',
                    background: dataset.storageStatus === 'stored' ? '#d1fae5' : '#fef3c7',
                    color: dataset.storageStatus === 'stored' ? '#065f46' : '#92400e',
                    borderRadius: '0.25rem',
                    fontSize: '0.75rem',
                  }"
                  >{{ dataset.storageStatus === "stored" ? "已存储" : dataset.storageStatus }}</span
                >
    ...
    <div
      v-if="statistics && Object.keys(statistics.by_source).length > 0"
      class="panel"
    ...
    >
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 1rem">
        <div
          v-for="[source, count] in Object.entries(statistics.by_source)"
          :key="source"

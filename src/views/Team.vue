...
onMounted(async () => {
  [rawMembers.value, projects.value, teamGroups.value] = await Promise.all([
    getPersonalMembers(),
    getProjects(),
    getTeamGroups(),
  ]);
  ...
  <section class="section team-hero">
    <div class="team-hero-content">
      <div>
        ...
        <h1>团队展示</h1>
        <p>
          团队按照研究与工程方向划分为数据组、遥感嵌入模型组、遥感视觉语言模型组和温度模型组。每个成员围绕所属方向承担具体的数据、模型、平台与应用任务。
        </p>
      </div>
      <div class="team-hero-stats">
        ...
        <div class="team-stat">
          <strong>{{ rawMembers.length }}</strong
          ><span>成员</span>
        </div>
        <div class="team-stat">
      ...
      <div class="member-list">
        <template v-if="group.members.length > 0">
          <MemberCard
            ...
            :key="member.id"
            :member="{ ...member, projects: resolveProjects(member.projects) as string[] | null }"
          />
        </template>
